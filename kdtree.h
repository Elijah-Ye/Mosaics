...
class KDTree
{
  ...
  private:
    ...
    KDTreeNode* KDTreeHelper(vector<Point<Dim>>&Lists, unsigned left, unsigned right, int dim);
    unsigned partition(vector<Point<Dim>>&Lists, unsigned left, unsigned right, unsigned pivotIndex, int dim);
    Point<Dim> select(vector<Point<Dim>>&Lists, unsigned left, unsigned right, unsigned k, int dim);
    void copy(KDTreeNode * root1, KDTreeNode * root2); 
    void clear(KDTreeNode *& root);
    Point<Dim> findNearestNeighbor(const Point<Dim>& query, int dimension, KDTreeNode * curRoot) const;
};
